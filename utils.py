...
def sent_to_vec(sent, tokenizer, model, pooling, max_length,device):
    with torch.no_grad():
...
def sents_to_vecs(sents, tokenizer, model, pooling, max_length, device,verbose=True):
    vecs = []
    if verbose:
        sents = tqdm(sents)
    for sent in sents:
        vec = sent_to_vec(sent, tokenizer, model, pooling, max_length,device)
        vecs.append(vec)
